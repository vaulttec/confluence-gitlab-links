...
public class TagMacro implements Macro {
	...
	public TagMacro(GitLabClient gitlabClient) {
	...
	public String execute(Map<String, String> parameters, String bodyContent, ConversionContext conversionContext)
		...
		if (StringUtils.isNotEmpty(url)) {
			...
			if (link.getType() == Type.TAG) {
				context.put("link", link);
			} else {
				context.put("error", "org.vaulttec.confluence-gitlab-links.tag.macro.error.invalid_url");
			}
		} else {
			context.put("error", "org.vaulttec.confluence-gitlab-links.tag.macro.error.no_url");
		}
		return VelocityUtils.getRenderedTemplate("templates/tag-macro.vm", context);
	}
