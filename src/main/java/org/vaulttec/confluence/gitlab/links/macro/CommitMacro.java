...
public class CommitMacro implements Macro {

	private final UserManager userManager;
	private final GitLabClient gitlabClient;

	public CommitMacro(UserManager userManager, GitLabClient gitlabClient) {
	...
	public String execute(Map<String, String> parameters, String bodyContent, ConversionContext conversionContext)
		...
		if (StringUtils.isNotEmpty(url)) {
			...
			if (link.getType() == Type.COMMIT && StringUtils.isNotEmpty(link.getName())) {
				context.put("link", link);

				// Only access GitLab API if API key is provided
				if (gitlabClient.hasApiKey()) {

					// Get commit details as current authenticated Confluence user
					...
					if (userProfile != null) {
						...
						if (commit != null) {
							...
							context.put("error",
									"org.vaulttec.confluence-gitlab-links.commit.macro.error.not_accessible");
						}
					}
				} else {
					context.put("error", "org.vaulttec.confluence-gitlab-links.commit.macro.error.not_accessible");
				}
			} else {
				context.put("error", "org.vaulttec.confluence-gitlab-links.commit.macro.error.invalid_url");
			}
		} else {
			context.put("error", "org.vaulttec.confluence-gitlab-links.commit.macro.error.no_url");
		}
		return VelocityUtils.getRenderedTemplate("templates/commit-macro.vm", context);
	}
